...
class SynthesizerModified(object):
    def __init__(
        self,
        tts_checkpoint: str = "",
        tts_config_path: str = "",
    ...
    def tts(
        ...
        sentence_id: str = "",
        tts_type: int = 0, # 0 for audio only 1 for dictionary with duration and sentence id
    ) -> List[int]:
        ...
        _output = {
            # "audio": wavs.tolist(),
            # "audio": wavs,
            # "audio": json.dumps(wavs, default=lambda x: float(x) if isinstance(x, np.float32) else None),
            "audio" : np.asarray(wavs, dtype=np.float32).tolist(),
            "duration":audio_time,
